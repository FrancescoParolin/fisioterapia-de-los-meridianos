    ...
    <>
      ...
      <section className="bg-foreground border-b border-foreground/80">
        <div className="mx-auto max-w-6xl px-6 py-24 md:py-32 grid md:grid-cols-2 gap-16 items-center">
          <AnimatedSection>
            <p className="text-xs uppercase tracking-[0.22em] mb-6" style={{ color: "var(--color-accent)" }}>
              {b.bodyLabel}
            </p>
            <h2 className="font-display text-4xl md:text-5xl leading-tight mb-7" style={{ color: "var(--color-background)" }}>
              {b.bodyH2}
            </h2>
            <div className="space-y-4 text-sm leading-relaxed" style={{ color: "oklch(from var(--color-background) l c h / 0.6)" }}>
              <p>{b.bodyP1}</p>
              <p>{b.bodyP2}</p>
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection delay={0.15}>
            <div className="relative">
              <div
                ...
                style={{ fontSize: "9rem", color: "oklch(from var(--color-accent) l c h / 0.25)", lineHeight: 1 }}
                aria-hidden="true"
              >
                &ldquo;
              </div>
      ...
      <section className="border-b border-border/60 bg-background">
        <div className="mx-auto max-w-5xl px-6 py-24">
          <AnimatedSection>
            ...
            <h2 className="font-display text-4xl md:text-5xl leading-tight mb-12 max-w-2xl">{b.whatH2}</h2>
          </AnimatedSection>
          <div className="grid md:grid-cols-5 gap-10 items-start">
            <AnimatedSection className="md:col-span-3" delay={0.1}>
              <div className="space-y-4 text-base leading-relaxed" style={{ color: "oklch(from var(--color-foreground) l c h / 0.72)" }}>
                <p>{b.whatP1}</p>
                <p>{b.whatP2}</p>
              </div>
            </AnimatedSection>
      ...
      <section className="border-b border-border/60 bg-card">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <AnimatedSection className="mb-16 max-w-lg">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            />
            {steps.map((s, i) => (
              <AnimatedItem key={s.n} delay={i * 0.12}>
      ...
      >
        <div className="mx-auto max-w-5xl px-6 py-24">
          ...
          <div className="flex flex-wrap gap-3">
            {outcomes.map((o, i) => (
              <AnimatedItem key={o.text} delay={i * 0.08}>
                <div className="flex items-center gap-3 rounded-full px-5 py-3 bg-background border border-border/60 hover:border-accent/40 transition-colors shadow-sm">
        ...
        <div className="mx-auto max-w-4xl px-6 py-24">
          ...
          <AnimatedSection delay={0.15}>
            <div className="max-w-sm mx-auto rounded-3xl bg-foreground shadow-2xl overflow-hidden">
              <div className="px-8 py-10 text-center border-b" style={{ borderColor: "oklch(from var(--color-background) l c h / 0.1)" }}>
                <div className="font-display leading-none mb-2" style={{ fontSize: "6rem", color: "var(--color-accent)" }}>
                  {b.priceAmount}
                </div>
                <div className="flex justify-center gap-4 text-sm mt-3" style={{ color: "oklch(from var(--color-background) l c h / 0.45)" }}>
              ...
              <div className="px-8 py-7 space-y-4">
                {[b.priceInc1, b.priceInc2, b.priceInc3].map((inc) => (
                  <div key={inc} className="flex items-center gap-3 text-sm" style={{ color: "oklch(from var(--color-background) l c h / 0.7)" }}>
      ...
      {/* ── TESTIMONIANZE ────────────────────────────────────── */}
      <section className="border-b border-border/60" style={{ background: "oklch(from var(--color-accent) l c h / 0.05)" }}>
        <div className="mx-auto max-w-6xl px-6 py-24">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <AnimatedItem key={t.name} delay={i * 0.13}>
                <div className="flex flex-col gap-4 rounded-2xl bg-background border border-border/60 px-7 py-8 shadow-sm h-full">
                  <div
                    className="font-display select-none leading-none"
                  ...
                  <blockquote className="flex-1 text-sm leading-relaxed italic" style={{ color: "oklch(from var(--color-foreground) l c h / 0.72)" }}>
                    {t.quote}
                  </blockquote>
                  ...
                  <div className="flex items-center gap-3 border-t border-border/50 pt-5">
                    <div
                      ...
                      style={{ background: `oklch(from var(--color-accent) l c h / ${0.12 + i * 0.04})`, color: "var(--color-accent)" }}
                    >
                      {t.name[0]}
                    </div>
                    <div>
      ...
      <section className="relative overflow-hidden bg-foreground border-b border-foreground/60">
        <div
          ...
          style={{ borderRadius: "60% 40% 70% 30% / 50% 60% 40% 60%", background: "oklch(from var(--color-accent) l c h / 0.14)", filter: "blur(60px)" }}
        />
        <div
          className="pointer-events-none absolute -bottom-24 -left-24 w-[320px] h-[320px]"
          style={{ borderRadius: "40% 60% 30% 70% / 60% 40% 70% 30%", background: "oklch(from var(--color-accent) l c h / 0.09)", filter: "blur(45px)" }}
        />
        <div className="relative mx-auto max-w-3xl px-6 py-28 md:py-40 text-center">
          <AnimatedSection>
            <p className="text-xs uppercase tracking-[0.25em] mb-6" style={{ color: "oklch(from var(--color-accent) l c h / 0.8)" }}>
              {b.callLabel}
            </p>
            <h2
            ...
            <p className="leading-relaxed mb-12 max-w-xs mx-auto text-sm" style={{ color: "oklch(from var(--color-background) l c h / 0.5)" }}>
              {b.callSubtitle}
            </p>
            <Link
              href="/prenota"
              className="inline-flex items-center gap-2.5 rounded-full px-9 py-4 font-medium transition-all hover:scale-105 text-base"
              style={{ background: "var(--color-accent)", color: "var(--color-background)" }}
            >
              {b.callCta}
